...
// Cambia la tarjeta asociada a una suscripción existente. Si la suscripción
// todavía está "pending" (nunca se autorizó, por ej. porque no se completó
// el checkout de MercadoPago), hay que mandar además status: "authorized"
// en el mismo request para que quede activa y arranque a cobrar — si no,
// MercadoPago guarda la tarjeta pero no ejecuta ningún cobro.
export async function actualizarTarjetaPreapproval(id, cardTokenId, { activar } = {}) {
  const body = { card_token_id: cardTokenId };
  if (activar) body.status = "authorized";
